...
These were modified separately and then a new dataset containing 1451 observations and 79 variables was created. More details of how we perform data data wrangling and deal with missing values are showed in the rmd file.
...
We pick variables that increase MSE if variable is randomly permuted greater than 10% (exclude two variables which are X1ndFlrSF and X2ndFlrSF which representing First/Second floor square feet, and are included in GrLivArea)。 Therefore, we choose to include total 5 varibales: GarageCars+ BsmtFinSF1+ OverallQual + Neighborhood + GrLivArea in our models.
...
Train the Vector Generalized Linear Model and summarize the prediction results of training set
```{r, Vector Generalized Linear Models - train}
library(VGAM)
...
```{r, Vector Generalized Linear Models- test}
...
Train the Linear Discriminant Analysis Model and summarize the prediction results of training set
```{r lda}
library(MASS)
...
Train the Support Vector Machine Model and summarize the prediction results of training set
```{r svm}
library(e1071)
...
Train the Random Forest Model and summarize the prediction results of training set
```{r rf predict}
...
Summarize the prediction results of training set
```{r}
# make predictions and summarize accuracy - testing set
...
For training set, as the Vector Generalized Linear classifier attempted to predict 'Lower' class most of the time, better performance in metrics for 'Lower' was achieved while the other two categories were poorly predicted. For testing set,Vector Generalized Linear Model best performed in 'Lower' category while poorly performed in two other categories. There were also a trade off between 'Median' and 'Higher' groups as the recall for 'Medium' group is higher(0.81) but the precission(0.69) is poorer, just opposite to 'Higher' which has higher precision(0.84) but lower recall(0.72). As consequence, it results only a fair overal prediction accuracy.
...
For training set, as the Linear Discriminant Analysis classifier attempted to predict 'Lower' class most of the time, better performance in metrics for 'Lower' was achieved while the other two categories were much poorly predicted. For testing set, Linear Discriminant Analysis Model best performed in 'Lower' category while poorly performed in two other categories. As consequence, it results a poor overal prediction accuracy.
...
For testing set, Support Vector Machine Model best performed in 'Lower' category while poorly performed in two other categories. There were also a trade off between 'Median' and 'Higher' groups as the recall for 'Higher' group is higher(0.84) but the precission(0.78) is poorer, just opposite to 'Median' which has higher precision but lower recall.
...
For testing set, Random Forest Model best performed in 'Lower' category while poorly performed in two other categories as precission for 'Median' and 'Higher' groups are around 0.7 ~ 0.8.
