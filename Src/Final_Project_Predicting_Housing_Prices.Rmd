...
In this consulting project, our main goal is to advise our client on where
is the best neighborhood to buy a house and how much a house he likes may cost.
Our client, a professor at the Utah State University, approached our team seeking for advice on
...
within the city of Ames, IA and he is not interested buying anywhere else. The client
has a price preference for houses between $180,000 - 210,000.

To best attend to our client's request, we explored the housing data on Ames, IA 
through different visualizations and a correlation plot to understand the dataset
[^1]. Then different inference models were constructed to verify our hypothesis. 
...
#install.packages(c("bartMachine","caret","corrplot","corrplot","corrplot","devtools","dplyr","e1071","flam","gam","ggfortify","ggplot2","ggrepel","glmnet","gridExtra","here","ISLR","knitr","lattice","mice","pander","psych","purrr","randomForest","RColorBrewer","readr","Rmisc","rsconnect","scales","shiny","stargazer","tidyr","tree","VGAM","VIM"))
...
# combine the data in the order of training + testing in order to prepare for EDA
...
These were modified separately and then a new dataset containing 1451 observations and 79 variables was created. More details of how we perform data wrangling and deal with missing values are shown in the rmd file.
...
```{r dealing with NAs 2, message=FALSE, warning=FALSE}
...
```{r dealing with NAs 3, message=FALSE, warning=FALSE}
...
In this section, we investigated the relationship between House sales Price and distance
from Utah State University. The distance between the house is an important
factor to our client. The professor emphasized that at this stage he wants to find
out if it is better to buy a house near campus or not. 

### 2.1 Hypothesis

Our initial hypothesis is that there is a positive association between
the final sales price and distance from ISU. In other words, houses that are
further away from the university are, in general, sold for a higher price than those
in neighborhoods closer to ISU. Without doing any data analysis, we know 
that there is a golf course at the opposite end of the city from the University, 
...
df22 <- data.frame(tapply(d_train_noNAs$OverallCond, d_train_noNAs$Neighborhood, mean))
...
- have the highest average sales price, \$300,000 or more. We classified these three
neighborhoods as "High", in regards to their average sales price. Similarly,
we identified two other strata of neighborhoods one as "Medium" with the average
sales price between $150,000 and \$300,000, and "Low" all neighborhoods with average
sales price up to $150,000.

<center>
```{r, message=FALSE, warning=FALSE}
#plot the results
ggplot(neighb) + 
  geom_col(aes(neighb$rn, y = neighb$mean.price,  fill = df$rn)) + 
  coord_flip() + 
...
```
 <center>
 `Figure 1.1 summarizes the the average sales price for each neighborhood in Ames, Iowa.`
...
```
 ...
 `Figure 1.2 summarizes the number of houses and their average price per neighborhood.`

```{r}
#Distribution of houses per neighborhood category
pander(table(d_train_noNAs$Neighborhood, d_train_noNAs$Mean.price))
```
`Table 2: Number of houses by price category and neighborhood`

</center>

###2.3 Hypothesis
  
 As seen in the 'Context' section, the city of Ames is a college city, with approximately 35,993 students (excluding postdocs), accounting for more than 57% of the population. This is a conservative estimate since we are not including the university's staff. Arguably, at least 60% of Ames works and/or studies at the Iowa State University.

`Then, what is the relationship between the distance from the university and the final sales price of a house?`
...
```{r SalesPrice, message=FALSE, warning=FALSE}
summary(all$SalePrice)
sp <- all$SalePrice / 1000
min(sp)
hist(sp, 
     main = 'Histogram of Sale Price',
     xlab = 'Sale Price',
     breaks = seq(0, 760, 10),prob = TRUE)
lines(density(sp), col = "blue", lwd = 2)       
lines(density(sp, adjust = 2), lty = "dotted")

```
 <center>
 `Figure 2.1 summarizes the frequency of Sales Price for all houses.`
</center>
...
Overall Quality is observed to have a positive correlation with SalePrice. It does not seem to have noticeable outliers apart from a point in level 4. 
...
```
 <center>
`Figure 2.2 summarizes the relationship of Sales Price and Overall Quality Rating of houses.`
</center>
...
```{r SalesPrice and Condition, message=FALSE, warning=FALSE}
...
```
 <center>
 `Figure 2.3 summarizes the relationship of Sales Price and Overall Condition Rating of houses.`
...
GrLivArea is observed to have a positive correlation with SalePrice, which make sense since big houses are generally more expensive than smaller. One thing worth noticing is that there are two houses with really large living areas and low SalePrice (potential to be outliers). I checked the data and found out they are houses 524, 1299 and 2550(NA for SalePrice).
...
```
 <center>
 `Figure 2.4 summarizes the relationship of Sales Price and Ground Living Area of houses.`
</center>

```{r, message=FALSE, warning=FALSE}
# potential outliers: houses with large GrLivArea but relatively low Sales Price 
outlier1 <- all %>%  filter(GrLivArea > 4500)

# checking the Overall Quanlity rating of these points to see if the low sales pricr is due to poor quality
...
The Size of garage shows a positive correlation with SalePrice from level 0-3 but fall at level 4 (non-linear correlation). It makes sense because only big houses have a large garage, and bigger houses usually have higher SalePrice. Also for houses with a 3 cars garage (level 3), there are several outliners with much higher SalePrice existing, it must be other factors affecting the outcome price. Something might happen for houses with level 4 garage that needed to be explained.
...
```
 <center>
 `Figure 2.5 summarizes the relationship of Sales Price and Capacity of Garage of houses.`
</center>
...
```
<center>
`Figure 2.6 summarizes the relationship of Sales Price and Basement Area of houses.`
</center>
...
<p>YearBuilt: Original construction date.</p>
...
<center>
`Figure 2.7 summarizes the relationship of Sales Price and Year of house built.`
</center>

<br>


### 2.5 Inferential Models

We will investigate the validity of our hypothesis by using Ordinary Least 
Squares to generate three different models.


<h4> 2.5.1 Ordinary Least Squares </h4>

First, we ran a naive OLS model including only the variables of interest in the model
. The model had statistical significance with an F-statistic of 26.33 and 
...
`Table 4: Comparing results from three models`
...
`Table 5: Results from using all variables in OLS`
...
```{r Lasso Cross Validation, message=FALSE, warning=FALSE}
...
```
 ...
 <center>
 `Figure 3.2: Cross Validation for Lasso`
 
 `Table 6: Lasso Coefficients`
...
The result from Model 3 provides evidence against the initial hypothesis that there is a positive association between the final sales price and distance from ISU. In fact, the model reveals the opposite, on average, for each additional minute it takes from a neighborhood to ISU campus is associated with __a price drop of approximately $615.7__ in the final sales price. Although the result is statistically significant at the 0.05 level, the result was not substantive. Therefore, we can conclude that distance from the ISU campus is not a statistically important factor when determining the house/apt sales price. So our
the client should feel free to select a house regardless of its distance to ISU campus.
...
According to the correlation table, variable 'Overall Quality'(Overall material and finish quality), the 'GrLivArea' (‘Above Grade’ Living Area) and 'GarageCars'(Size of the garage in car capacity) are the top 3 variables that highly correlated to SalePrice.
...
```{r correlation2, message=FALSE, warning=FALSE}
...
```
 <center>
 `Figure 4: Correlation Table of Sales Price and all numeric variables`
 </center>
 
Since there are several paris of varibles are highly correlated to each other, we can simply drop one of them.Therefore, I chose to drop 'TotalRmsAbvGrd'(vs GrLivArea); 'GarageArea'(vs GarageCars); 'TotalBsmtSF' (vs X1stFkrSF); 'YearRemodAdd'(vs YearBiult)

```{r drop variables and outliers, message=FALSE, warning=FALSE}
dropVars <- c('YearRemodAdd', 'GarageArea', 'TotalBsmtSF', 'TotalRmsAbvGrd')

train <- train[,!(names(train) %in% dropVars)]

train <- train[-c(524, 1299, 2550),] # removig outliers
write_csv(train, 'Final_data.csv') # save to a new file
```

### 3.2 Random Forest

The correlation table above provides a good overview of the most important numeric variables and multicollinearity among those variables. Next, I want to get an overview of the most important variables including the categorical variables.

```{r rf again, message=FALSE, warning=FALSE}
...
```
 <center>
 `Figure 5: Varaible selection by random forest`
 </center>
 
### 3.3 Predictive Models

We pick variables that increase MSE if variable is randomly permuted greater than 10% (exclude two variables which are X1ndFlrSF and X2ndFlrSF which representing First/Second floor square feet, and are included in GrLivArea)。 Therefore, we choose to include total 5 variables: GarageCars+ BsmtFinSF1+ OverallQual + Neighborhood + GrLivArea in our models.

For model evaluation, since our class labels are not uniformly distributed, we decided to focus on four key metrics:
1) 'Overall classification accuracy' which is defined as the fraction of instances that are correctly classified;
2) 'Precision' which is defined as the fraction of correct predictions for a certain class;
3) 'Recall' which is the fraction of instances of a class that were correctly predicted;
4) 'F1 score' which is defined as the harmonic mean (or a weighted average) of precision and recall.

```{r ols, message=FALSE, warning=FALSE}
...
```{r, message=FALSE, warning=FALSE}
# removing unnecessary varibles and factor levels
...
#### 3.3.1 Vector Generalized Linear Model
Train the Vector Generalized Linear Model and summarize the prediction results of the training set
```{r, message=FALSE, warning=FALSE}
...
Summarize the prediction results of the Testing set
...
Train the Linear Discriminant Analysis Model and summarize the prediction results of the training set
...
Train the Support Vector Machine Model and summarize the prediction results of the training set
...
data.frame(precision, recall, f1) 
```
Summarize the prediction results of testing set
```{r, message=FALSE, warning=FALSE}
...
Train the Random Forest Model and summarize the prediction results of the training set
```{r rf predict, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
Summarize the prediction results of the training set
```{r, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
# make predictions and summarize accuracy - testing set
...
The prediction accuracy of Vector Generalized Linear Model in the testing set is 0.8997135.
For training set, as the Vector Generalized Linear classifier attempted to predict 'Lower' class most of the time, better performance in metrics for 'Lower' was achieved while the other two categories were poorly predicted. For the testing set, Vector Generalized Linear Model best performed in 'Lower' category while poorly performed in two other categories. There was also a trade-off between 'Median' and 'Higher' groups as the recall for 'Medium' group is higher(0.81) but the precision(0.69) is poorer, just opposite to 'Higher' which has higher precision(0.84) but lower recall(0.72). As consequence, it results in only a fair overall prediction accuracy.

<br>

####Linear Discriminant Analysis Model
The prediction accuracy of Linear Discriminant Analysis Model in training set is 0.8424908.
The prediction accuracy of Linear Discriminant Analysis Model in the testing set is 0.8681948.
For training set, as the Linear Discriminant Analysis classifier attempted to predict 'Lower' class most of the time, better performance in metrics for 'Lower' was achieved while the other two categories were much poorly predicted. For the testing set, Linear Discriminant Analysis Model best performed in 'Lower' category while poorly performed in two other categories. As consequence, it results in a poor overall prediction accuracy.

<br>

####Support Vector Machine Model
The prediction accuracy of Support Vector Machine Model in training set is 0.9798535.
The prediction accuracy of Support Vector Machine Model in the testing set is 0.9169054.
For the testing set, Support Vector Machine Model best performed in 'Lower' category while poorly performed in two other categories. There was also a trade-off between 'Median' and 'Higher' groups as the recall for 'Higher' group is higher(0.84) but the precision(0.78) is poorer, just opposite to 'Median' which has higher precision but lower recall.

<br>

#### Random Forest Model
The prediction accuracy of Random Forest Model in training set is 0.9945055.
The prediction accuracy of Random Forest Model in the testing set is 0.8939828.
For training set, Random Forest Model gave the best fitting in all three groups 
For the testing set, Random Forest Model best performed in 'Lower' category while poorly performed in two other categories as precision for 'Median' and 'Higher' groups are around 0.7 ~ 0.8.

<br>

Lastly, Vector Generalized Linear Model and Linear Discriminant Analysis Model gave results with fair accuracy and no obvious underfitting was observed. The Support Vector Machine Model gave the best accuracy in testing set (0.917) and has small overfitting problem in training set (0.980). The Random Forest Model gave the best accuracy in the training set (0.995) almost 100%, but significant overfitting of training set was observed as the accuracy of the testing set is only 0.894. Therefore, we concluded that the Support Vector Machine Model is our best predictive model.

***

## 4. Conclusion

In this consulting project, our main goal is to advise our client on where is the best neighborhood to buy a house and how much a house he likes may cost. After analyzing the housing data from the city of Ames, IA, we are confident that the best neighborhoods for our client, are  College Creek and Crawford. Both neighborhoods are within 5 minutes from ISU campus and have an average price of \$197,000 and $210,000 respectively. Both neighborhoods have an overall quality above average. Houses in Crawford have an overall condition way above the average although the journey to campus by bus is a bit longer than College Creek. College Creek houses, on average, has an okay overall condition. Table XX summarizes the findings:

<center>

`Table 7: Recommended Neighboorhoods for the Client`

| Code    | FullName      | Car.Time.min | Walk.time.min | Bus.time.min | Avg Final Price | Overall Qual | Overall Cond |
|---------|---------------|--------------|---------------|--------------|-----------------|--------------|--------------|
| CollgCr | College Creek | 5            | 12            | 14           | $197,703.80     | 6.63         | 5.241611     |
| Crawfor | Crawford      | 5            | 20            | 18           | $210,824.74     | 6.28         | 6.66         |

</center>

Furthermore, our inference models indicate that distance from campus is not a significant predictor on final sales price of the house. In other words, although the relationship between the final sales price and distance from ISU campus proved to be statistically significant at the 0.05 level, the result is __somewhat substantive__ as it only changes around 3% (the closest house to campus) - 18% (the furthest from campus) of the final sales price of the house (. The client should feel free to choose the location of his house according to his own preferences without worrying about the effect on the final sales price. 

Lastly, we performed predictive analysis of the sales price of a house. By analysis, we found that there are five most important factors that affect the final sales price of a house: internally, all of the Overall Quality of the house; the above ground living area, the basement area and the garage capacity of the house affect the final sales price; externally, which neighbourhood the house in Ames, IA the house located in matters quite a lot on the final sales price. 
As for predictive model, we applied data into four different types of classification models (Vector Generalized Linear Model, Linear Discriminant Analysis Model, Suppoprt Vector Machine Model and Random Forest Models) and found out the support vector machine model gave the best predictive result in testing data (accuracy = 0.9169) among all. 


Should the client choose to continue with the project the following next steps are essential to improve our results and provide further guidance in this important purchase:

1) Time Series Analysis on Pricing
2) Cluster Analysis on Neighborhood
3) Specific client data
4) Analyze city-wide plans and developments that could affect our models and forecasting.
